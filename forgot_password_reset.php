...
if(isset($_GET['key']) && isset($_GET['email'])) {
    $key=$_GET['key'];
    $email=$_GET['email'];
    $check=mysqli_query($database,"SELECT * FROM forget_password WHERE email='$email' and temp_key='$key'");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if ($password2==$password1) {
            ...
            mysqli_query($database,"DELETE FROM forget_password where email='$email' and temp_key='$key'");
            //update password in database
            mysqli_query($database,"UPDATE patient set ppassword='$password' where pemail='$email'");
        }
        else{
