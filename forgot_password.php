...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    $email_reg=mysqli_real_escape_string($database,$_POST['email']);
    $details=mysqli_query($database,"SELECT pemail FROM patient WHERE pemail='$email_reg'");
    if (mysqli_num_rows($details)>0) { //if the given email is in database, ie. registered
        ...
        $sql_insert=mysqli_query($database,"INSERT INTO forget_password(email,temp_key) VALUES('$email_reg','$key')");
        //sending email about update
        $to      = $email_reg;
        $subject = 'Changing password DEMO- admin@wecreatesmile.com';
        $msg = "Please copy the link and paste in your browser address bar". "\r\n"."https://wecreatesmile.com/forgot_password_reset.php?key=".$key."&email=".$email_reg;
